...
export async function extract({ file, schema, template, model }) {
  try {
    ...
    if (template) {
      ...
      throw new Error("You must provide a schema or a template.");
    }
    ...
    const extractionResult = await extraction({
      markdown,
    ...
    const event = extractionResult?.data !== undefined ? extractionResult.data : extractionResult;
    const extractUsage = extractionResult?.usage || null;
